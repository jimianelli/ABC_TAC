...
#goaraw <- read_csv(here("data", "GOA_OFL_ABC_TAC.csv"), show_col_types = FALSE)
goaraw <- read_csv(here("data", "GOA_OFL_ABC_TAC_specs.csv"), show_col_types = FALSE)
...
```{r}
#| label: fig-cv-goa
#| fig-cap: "Interannual variability (CV) of ABC and TAC for selected GOA species (top seven by mean ABC; lag 1, OY = 1, Area = Total)."
...
# Predict TAC from a full set of ABC inputs (including the focal stocks).
# This is closer to the intended "replace ATTACH" use case.
dsem_indep <- mainspp
...
# Robust fit across dsem versions:
# 1) try chosen gmrf with default family
# 2) if needed, retry with family = "gaussian"
# 3) then cycle other gmrf options.
...
#
# IMPORTANT: the DSEM forecast requires *assumed future ABCs* as inputs.
# Below we create a simple demonstrator scenario by carrying forward the last
# observed ABCs and applying small annual changes (edit these as needed).

n_future <- 5
set.seed(1)

# Last observed ABC vector (lag 1, OY = 1) used as the baseline.
last_abc <- as.numeric(dsem_df[nrow(dsem_df), x_cols])
names(last_abc) <- x_cols

# Scenario: modest annual increases for Pollock and Pacific cod ABC, slight
# decline for others, plus small random noise.
growth <- rep(0.00, length(x_cols))
names(growth) <- x_cols
if ("ABC_Pollock" %in% x_cols) growth["ABC_Pollock"] <- 0.02
if ("ABC_Pacific_cod" %in% x_cols) growth["ABC_Pacific_cod"] <- 0.01

other_cols <- setdiff(x_cols, c("ABC_Pollock", "ABC_Pacific_cod"))
growth[other_cols] <- -0.005
...
# Combine assumed ABCs and predicted TACs for a compact forecast table.
...
TAC time series and (2) contemporaneous influence of ABC inputs (including both focal-stock ABC and cross-stock ABCs). The model is fit on scaled 
inputs for stability, then predictions are transformed back to the original TAC units for interpretation.

To make the forecasting intent concrete, the setup chunk above also constructs a simple example set of future ABC inputs and returns a `forecast_tbl` object containing assumed ABCs and the corresponding predicted TACs.

```{r}
#| label: tbl-dsem-forecast
#| tbl-cap: "Example DSEM forecast: assumed future ABCs and predicted TACs (illustrative scenario)."
forecast_tbl %>%
