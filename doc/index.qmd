...
ebs_pollock_catch <- read_csv(file.path(data_root, "Catch1991-2025.csv"), show_col_types = FALSE)
...
#| fig-cap: "BSAI totals by year (sum across species): TAC, ABC, and OFL, with EBS pollock catch overlaid from 1991-2025."
#| fig-height: 4.5
#| fig-width: 7
...
bsaiyr <- dfOY %>%
  ...
  left_join(ebs_pollock_catch, by = "Year")

plot_bsai <- bsaiyr %>%
  select(Year, TAC, ABC, OFL, Catch) %>%
  pivot_longer(-Year, names_to = "Measure", values_to = "value") %>%
  mutate(
    value = na_if(value, 0),
    Measure = recode(Measure, Catch = "EBS pollock catch"),
    Measure = factor(Measure, levels = c("TAC", "ABC", "OFL", "EBS pollock catch"))
  )
